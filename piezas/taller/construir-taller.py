...
HELMET = open(f'{BANCO}/fuentes/Main.dc.html').read().split('<helmet>')[1].split('</helmet>')[0]
CREDITOS = json.load(open(f'{BANCO}/fotos/creditos.json'))
FOTOS = list(CREDITOS.keys())                      # las once del banco, en su orden
LUCIDE = json.load(open(f'{BANCO}/iconos/lucide-kit.json'))

# ── colores de la casa (mobile/src/theme/tokens.ts del repo de la app, copiados a mano) ──
...
# ═══════════════════════════════════════════════════════════════════════════════
# 1 · IMÁGENES. Las fotos de historia (1080×1920, 9:16) ya viven recortadas en
#     banco/fotos/historia — recortadas del original de Commons, no de la de 1080×1350;
#     las tres postales salían de recortes de 980×380 ampliados cinco veces y por eso.
# ═══════════════════════════════════════════════════════════════════════════════
peso = 0
for f in FOTOS:
    shutil.copy(f'{BANCO}/fotos/historia/{f}', OUT); peso += os.path.getsize(f'{OUT}/{f}')
CAPTURAS = ['plan', 'tour', 'museo', 'grupo', 'crear', 'visita']
for c in CAPTURAS:
    for suf in ('', '-it'):
        shutil.copy(f'{BANCO}/capturas/{c}-900{suf}.webp', OUT); peso += os.path.getsize(f'{OUT}/{c}-900{suf}.webp')
for src in (f'{BANCO}/marca/mark.png', f'{BANCO}/marca/mark-claro.png', f'{BANCO}/qr/tarjeta-qr-ejemplo.png'):
    shutil.copy(src, OUT); peso += os.path.getsize(src)
...
nota('nota-reel', 4640, -330, 520, 'Una escena de reel es un PNG más. El reel entero son cinco: se exportan y se montan con piezas/reels/montar-reel.sh, que pone el movimiento y los fundidos.', 'plantillas')
...
nota('nota-montaje', 0, -330, 520, 'Cada fila es un reel: cinco escenas que se exportan a PNG y se montan con piezas/reels/montar-reel.sh. Sin audio a propósito: se elige dentro de Instagram al subir.', 'reels')
